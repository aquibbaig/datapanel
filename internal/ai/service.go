...
func systemPrompt(dialect string, responseStyle string) string {
	...
	lines := []string{
		...
		"Treat table confidence as a ranking: use the highest-confidence table that satisfies the request, and use lower-confidence related tables only when their DDL is needed for the requested result.",
...
func planPrompt(dialect string) string {
	...
	return strings.Join([]string{
		...
		"Include both directly requested tables and plausible supporting or closely related tables.",
		"Use confidence near 1.0 for a direct subject match and lower confidence for related candidates (for example 0.2 for a weak semantic relation).",
		"Explain each candidate's relationship in reason; do not omit a clear direct match because related candidates also exist.",
		...
		"Return at most five ranked candidates. Include join tables when the request implies a join, and keep merely related tables at low confidence.",
		"The SQL dialect is " + normalizedDialect + ".",
	}, "\n")
}
